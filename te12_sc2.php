...
<form name=scheda method=post action=te12_scheda.php?zid=<?php echo urlencode($id); ?>  onSubmit="return verScheda(document.scheda);">
...
$result=MYSQL_QUERY("SELECT tc_mid, tc_team1, tc_team2, UNIX_TIMESTAMP(tc_date) as ladata, tc_group FROM te12_match, te12_teams WHERE tc_tid=tc_team1 ORDER BY tc_date");
if ($myrow = mysql_fetch_array($result)) {
	do{ $sijuega= date("d/m/Y H:i", $myrow["ladata"]);
$resultd=MYSQL_QUERY("SELECT  tc_g1, tc_g2, tc_1x2 FROM te12_res WHERE tc_rpid='$id' AND tc_rmid='{$myrow['tc_mid']}'"); 
...
$optionx='<option selected value=' . $myrow2["tc_1x2"] .'>' .$myrow2["tc_1x2"] . '</option>';
	...
	<?php
$result2=MYSQL_QUERY("SELECT tc_tid FROM te12_teams ORDER BY tc_tid");
...
$allteams=$allteams . "<option value='" . $myrow["tc_tid"] . "'>". $myrow["tc_tid"] . "</option>";
	...
	<?php
		$result4=MYSQL_QUERY("SELECT tc_16tid FROM te12_16teams WHERE tc_16pid='$id'  AND tc_16w='4' ORDER BY tc_16tid");
...
?>
	...
	<?php
		$result4=MYSQL_QUERY("SELECT tc_16tid FROM te12_16teams WHERE tc_16pid='$id'  AND tc_16w='2' ORDER BY tc_16tid");
if ($myrow2 = mysql_fetch_array($result4)) {
	do{
...
?>
	...
	<?php
		$result4=MYSQL_QUERY("SELECT tc_16tid FROM te12_16teams WHERE tc_16pid='$id'  AND tc_16w='F' ORDER BY tc_16tid");
if ($myrow2 = mysql_fetch_array($result4)) {
	do{
...
?>
	...
	<?php
		$result4=MYSQL_QUERY("SELECT tc_16tid FROM te12_16teams WHERE tc_16pid='$id'  AND tc_16w='W' ORDER BY tc_16tid");
if ($myrow2 = mysql_fetch_array($result4)) {
	do{
...
$result=MYSQL_QUERY("SELECT  tc_tid FROM te12_teams ORDER BY tc_tid");
...
do{
	...
	$resulto=MYSQL_QUERY("SELECT  tc_sid, tc_name1 FROM te12_stars WHERE tc_stid='{$myrow['tc_tid']}' ORDER BY tc_name1");
...
print("}</script>");} 
$result3=MYSQL_QUERY("SELECT tc_sid, tc_name1 FROM te12_stars ORDER BY tc_name1");
...
?>
	...
	<?php
	$result4=MYSQL_QUERY("SELECT tc_gsid,tc_name1 FROM te12_goleadors,te12_stars WHERE tc_gpid='$id' AND tc_gsid=tc_sid ORDER BY tc_name1");
if ($myrow2 = mysql_fetch_array($result4)) {
	do{
...
?>
	...
	<?php
$result4=MYSQL_QUERY("SELECT tc_fsid, tc_name1 FROM te12_final,te12_stars WHERE tc_fpid='$id' AND tc_fsid=tc_sid");
if ($myrow2 = mysql_fetch_array($result4)) {
	do{
