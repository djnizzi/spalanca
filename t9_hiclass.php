...
<table width="100%" border="0" cellspacing="1" cellpadding="1">
 <tr>
  ...
  <?php
  $result=MYSQL_QUERY("SELECT count(*) as segnari FROM t9_playaz");
  $partecipantes= @MYSQL_RESULT($result,0,"segnari");
  $result=MYSQL_QUERY("SELECT tc_points, tc_pid FROM t9_playaz WHERE tc_paid='Y' ORDER BY tc_points DESC");
if ($myrow = mysql_fetch_array($result)) {
	do{
printf("<tr><td class=masblack valign=top><a href=t9_scheda.php?zid=%s>%s</a></td><td background=images/tc_bk.jpg align=right width=78><a href=t9_scheda.php?zid=%s><img src=getdata.php?zid=%s&wht=users border=0  width=50 height=50></a></td><td class=mascont align=center><b><font size=+3>%s</font></b><br>punti</td></tr>",urlencode($myrow["tc_pid"]),$myrow["tc_pid"],urlencode($myrow["tc_pid"]),urlencode($myrow["tc_pid"]),$myrow["tc_points"]);
...
<?php	
$result=MYSQL_QUERY("SELECT count(*) as segnari, tc_16tid FROM t9_16teams WHERE tc_16w='W'  GROUP BY tc_16tid  ORDER BY segnari DESC");
if ($myrow = mysql_fetch_array($result)) {
	do{
	$perc=(($myrow["segnari"]*100)/($partecipantes));
printf("<tr><td class=mastoxic align=right width=100><img src=t9_getdata.php?tid=%s></td><td class=mascont width=104><img src=images/white.gif  width=%s height=14></td><td class=mascont width=50 align=right>%s %%</td></tr>",urlencode($myrow["tc_16tid"]),round($perc,0),round($perc,2));
} while  ($myrow = mysql_fetch_array($result));} ?>	
</table></td></tr>
	<tr>
    <td class="mastoxic3" align=center><table border="0" cellpadding="3" cellspacing=1 align=center >
 ...
 <?php	
$result=MYSQL_QUERY("SELECT count(*) as segnari, tc_name1 FROM t9_goleadors, t9_stars WHERE tc_sid=tc_gsid  GROUP BY tc_gsid  ORDER BY segnari DESC LIMIT 10");
if ($myrow = mysql_fetch_array($result)) {
	do{
