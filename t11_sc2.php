...
<?php mysql_select_db($database2);

$result=MYSQL_QUERY("SELECT  tc_tid FROM t11_teams ORDER BY tc_tid");
...
do{
	...
	$resulto=MYSQL_QUERY("SELECT  tc_sid, tc_name1 FROM t11_stars WHERE tc_stid='{$myrow['tc_tid']}' ORDER BY tc_name1");
	if ($myrow2 = mysql_fetch_array($resulto)) {
		printf ("if (squadra.options[squadra.options.selectedIndex].value=='%s'){",$myrow["tc_tid"]);	
	do{
printf("var newone=new Option('%s','%s');giocatore.options.add(newone);   \n",$myrow2["tc_name1"],$myrow2["tc_sid"]);	
} while  ($myrow2 = mysql_fetch_array($resulto));print("}");
	} 		
	
}while  ($myrow = mysql_fetch_array($result));
print("}</script>");}    include ("t11_nav.php");
?>
<form name=scheda method=post action=t11_scheda.php?zid=<?php echo $id; ?>  onSubmit="return verScheda(document.scheda);">
...
$result=MYSQL_QUERY("SELECT tc_mid, tc_team1, tc_team2, UNIX_TIMESTAMP(tc_date) as ladata, tc_group FROM t11_match, t11_teams WHERE tc_tid=tc_team1 ORDER BY tc_date");
if ($myrow = mysql_fetch_array($result)) {
	do{ $sijuega= date("d/m/Y H:i", $myrow["ladata"]);
$resultd=MYSQL_QUERY("SELECT  tc_g1, tc_g2, tc_1x2 FROM t11_res WHERE tc_rpid='$id' AND tc_rmid='{$myrow['tc_mid']}'"); 
...
$optionx='<option selected value=' . $myrow2["tc_1x2"] .'>' .$myrow2["tc_1x2"] . '</option>';
	...
	<?php
$result2=MYSQL_QUERY("SELECT tc_tid FROM t11_teams ORDER BY tc_tid");
if ($myrow = mysql_fetch_array($result2)) {
	do{
$allteams=$allteams . "<option value='" . $myrow["tc_tid"] . "'>". $myrow["tc_tid"] . "</option>";
} while  ($myrow = mysql_fetch_array($result2));

	$result4=MYSQL_QUERY("SELECT tc_16tid FROM t11_16teams WHERE tc_16pid='$id'  AND tc_16w='8' ORDER BY tc_16tid");
...
?>
	...
	<?php
		$result4=MYSQL_QUERY("SELECT tc_16tid FROM t11_16teams WHERE tc_16pid='$id'  AND tc_16w='4' ORDER BY tc_16tid");
if ($myrow2 = mysql_fetch_array($result4)) {
	do{
...
?>
	...
	<?php
		$result4=MYSQL_QUERY("SELECT tc_16tid FROM t11_16teams WHERE tc_16pid='$id'  AND tc_16w='2' ORDER BY tc_16tid");
if ($myrow2 = mysql_fetch_array($result4)) {
	do{
...
?>
	...
	<?php
		$result4=MYSQL_QUERY("SELECT tc_16tid FROM t11_16teams WHERE tc_16pid='$id'  AND tc_16w='F' ORDER BY tc_16tid");
if ($myrow2 = mysql_fetch_array($result4)) {
	do{
...
?>
	...
	<?php
		$result4=MYSQL_QUERY("SELECT tc_16tid FROM t11_16teams WHERE tc_16pid='$id'  AND tc_16w='W' ORDER BY tc_16tid");
if ($myrow2 = mysql_fetch_array($result4)) {
	do{
...
$result3=MYSQL_QUERY("SELECT tc_sid, tc_name1 FROM t11_stars ORDER BY tc_name1");
...
?>
	...
	<?php
	$result4=MYSQL_QUERY("SELECT tc_gsid,tc_name1 FROM t11_goleadors,t11_stars WHERE tc_gpid='$id' AND tc_gsid=tc_sid ORDER BY tc_name1");
if ($myrow2 = mysql_fetch_array($result4)) {
	do{
$myG=$myG . "<option value='" . $myrow2["tc_gsid"] . "'>". $myrow2["tc_name1"] . "</option>";
} while  ($myrow2 = mysql_fetch_array($result4));}
print ("<tr><td  width=33% class=mascont align=center><select id=selectSG name=selectSG class=mastoxic onChange='dynsel(document.all.selectSG,document.all.selectG);'><option>seleziona squadra</option>$allteams</select><br><select class=mastoxic id=selectG name=selectG size=10 multiple>");
print ("</select></td><td  width=33% class=mastoxic align=center><input type=button class=buttons value='aggiungi --&gt;' ");
...
?>
	...
	<?php
$result4=MYSQL_QUERY("SELECT tc_fsid, tc_name1 FROM t11_final,t11_stars WHERE tc_fpid='$id' AND tc_fsid=tc_sid");
if ($myrow2 = mysql_fetch_array($result4)) {
	do{
$myX=$myX . "<option value='" . $myrow2["tc_fsid"] . "'>". $myrow2["tc_name1"] . "</option>";
} while  ($myrow2 = mysql_fetch_array($result4));}
print ("<tr><td  width=33% class=mascont align=center><select id=selectXG name=selectXG class=mastoxic onChange='dynsel(document.all.selectXG,document.all.selectX);'><option>seleziona squadra</option>$allteams</select><br><select class=mastoxic id=selectX name=selectX size=10 multiple>");
print ("</select></td><td width=33% class=mastoxic align=center><input type=button class=buttons value='aggiungi --&gt;' ");
