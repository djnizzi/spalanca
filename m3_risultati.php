...
<table width="100%" border="0" cellspacing="1" cellpadding="1">
 <tr>
  ...
  <?php
$result=MYSQL_QUERY("SELECT  sm_mid, sm_team1, sm_team2,  sm_g1, sm_g2, UNIX_TIMESTAMP(sm_date) as ladata, sm_group FROM m3_match, m3_teams WHERE sm_tid=sm_team1 AND sm_play='Y' ORDER BY sm_date");
if ($myrow = mysql_fetch_array($result)) {
	do{$gole="";
$xpu=""; $matchid=$myrow["sm_mid"];$sijuega= date("d/m/Y H:i", $myrow["ladata"]);
	$result2=MYSQL_QUERY("SELECT  sm_name1 FROM m3_goal, m3_stars WHERE sm_gmid=$matchid AND sm_sid=sm_gsid ORDER BY sm_name1");
	...
	$result2=MYSQL_QUERY("SELECT  sm_name1 FROM m3_sentoff, m3_stars WHERE sm_smid=$matchid AND sm_sid=sm_ssid ORDER BY sm_name1");
	...
	if ($myrow["ladata"]<mktime (0,0,0,6,26,2010)) {$whatmatch="Gruppo " . $myrow["sm_group"];} else 
	if ($myrow["ladata"]<mktime (0,0,0,7,01,2010)){$whatmatch="1/8 finale";} else
	if ($myrow["ladata"]<mktime (0,0,0,7,04,2010)){$whatmatch="1/4 finale";}else 
	if ($myrow["ladata"]<mktime (0,0,0,7,10,2010)){$whatmatch="semifinale";}else {$whatmatch="finale";}
	printf ("<tr><td class=mascont>%s</td><td class=mascont>%s</td>", $sijuega,$whatmatch);
$tim1 = $myrow["sm_team1"];
$tim2 = $myrow["sm_team2"];
    $query1res = @MYSQL_QUERY("SELECT sm_flagfn FROM m3_teams WHERE sm_tid='$tim1'");
    $query2res = @MYSQL_QUERY("SELECT sm_flagfn FROM m3_teams WHERE sm_tid='$tim2'");
  ...
  <?php
$result=MYSQL_QUERY("SELECT  sm_team1, sm_team2, UNIX_TIMESTAMP(sm_date) as ladata, sm_group FROM m3_match, m3_teams WHERE sm_tid=sm_team1 AND sm_play='N' ORDER BY sm_date");
if ($myrow = mysql_fetch_array($result)) {
...
<?php
$result=MYSQL_QUERY("SELECT  sm_tid,sm_flagfn, sm_gf, sm_ga, sm_gp, sm_points, (sm_gf-sm_ga) as dr FROM m3_teams WHERE sm_group='A' ORDER BY sm_points DESC, dr DESC, sm_gf DESC,  sm_tid");
...
<?php
$result=MYSQL_QUERY("SELECT  sm_tid, sm_flagfn, sm_gf, sm_ga, sm_gp, sm_points, (sm_gf-sm_ga) as dr FROM m3_teams WHERE sm_group='B' ORDER BY sm_points DESC, dr DESC, sm_gf DESC,  sm_tid");
if ($myrow = mysql_fetch_array($result)) { do {	
	printf ("<tr><td class=mastoxic><img src=tm/%s></td><td class=mascont align=center>%s</td><td class=mascont  align=center>%s - %s</td><td class=mascont  align=center><b>%s</td></tr>", urlencode($myrow["sm_flagfn"]), $myrow["sm_gp"],$myrow["sm_gf"],$myrow["sm_ga"],$myrow["sm_points"]);
...
<?php
$result=MYSQL_QUERY("SELECT  sm_tid,sm_flagfn, sm_gf, sm_ga, sm_gp, sm_points, (sm_gf-sm_ga) as dr FROM m3_teams WHERE sm_group='C' ORDER BY sm_points DESC, dr DESC, sm_gf DESC, sm_tid");
if ($myrow = mysql_fetch_array($result)) { do {	
	printf ("<tr><td class=mastoxic><img src=tm/%s></td><td class=mascont align=center>%s</td><td class=mascont  align=center>%s - %s</td><td class=mascont  align=center><b>%s</td></tr>", urlencode($myrow["sm_flagfn"]), $myrow["sm_gp"],$myrow["sm_gf"],$myrow["sm_ga"],$myrow["sm_points"]);
...
<?php
$result=MYSQL_QUERY("SELECT  sm_tid,sm_flagfn, sm_gf, sm_ga, sm_gp, sm_points, (sm_gf-sm_ga) as dr FROM m3_teams WHERE sm_group='D' ORDER BY sm_points DESC, dr DESC, sm_gf DESC, sm_tid");
if ($myrow = mysql_fetch_array($result)) { do {	
	printf ("<tr><td class=mastoxic><img src=tm/%s></td><td class=mascont align=center>%s</td><td class=mascont  align=center>%s - %s</td><td class=mascont  align=center><b>%s</td></tr>", urlencode($myrow["sm_flagfn"]), $myrow["sm_gp"],$myrow["sm_gf"],$myrow["sm_ga"],$myrow["sm_points"]);
...
<?php
$result=MYSQL_QUERY("SELECT  sm_tid,sm_flagfn, sm_gf, sm_ga, sm_gp, sm_points, (sm_gf-sm_ga) as dr FROM m3_teams WHERE sm_group='E' ORDER BY sm_points DESC, dr DESC, sm_gf DESC,   sm_tid");
if ($myrow = mysql_fetch_array($result)) { do {	
	printf ("<tr><td class=mastoxic><img src=tm/%s></td><td class=mascont align=center>%s</td><td class=mascont  align=center>%s - %s</td><td class=mascont  align=center><b>%s</td></tr>", urlencode($myrow["sm_flagfn"]), $myrow["sm_gp"],$myrow["sm_gf"],$myrow["sm_ga"],$myrow["sm_points"]);
...
<?php
$result=MYSQL_QUERY("SELECT  sm_tid,sm_flagfn, sm_gf, sm_ga, sm_gp, sm_points, (sm_gf-sm_ga) as dr FROM m3_teams WHERE sm_group='F' ORDER BY sm_points DESC, dr DESC, sm_gf DESC,   sm_tid");
if ($myrow = mysql_fetch_array($result)) { do {	
	printf ("<tr><td class=mastoxic><img src=tm/%s></td><td class=mascont align=center>%s</td><td class=mascont  align=center>%s - %s</td><td class=mascont  align=center><b>%s</td></tr>", urlencode($myrow["sm_flagfn"]), $myrow["sm_gp"],$myrow["sm_gf"],$myrow["sm_ga"],$myrow["sm_points"]);
...
<?php
$result=MYSQL_QUERY("SELECT  sm_tid,sm_flagfn, sm_gf, sm_ga, sm_gp, sm_points, (sm_gf-sm_ga) as dr FROM m3_teams WHERE sm_group='G' ORDER BY sm_points DESC, dr DESC, sm_gf DESC,   sm_tid");
if ($myrow = mysql_fetch_array($result)) { do {	
	printf ("<tr><td class=mastoxic><img src=tm/%s></td><td class=mascont align=center>%s</td><td class=mascont  align=center>%s - %s</td><td class=mascont  align=center><b>%s</td></tr>", urlencode($myrow["sm_flagfn"]), $myrow["sm_gp"],$myrow["sm_gf"],$myrow["sm_ga"],$myrow["sm_points"]);
...
<?php
$result=MYSQL_QUERY("SELECT  sm_tid,sm_flagfn, sm_gf, sm_ga, sm_gp, sm_points, (sm_gf-sm_ga) as dr FROM m3_teams WHERE sm_group='H' ORDER BY sm_points DESC, dr DESC, sm_gf DESC,   sm_tid");
if ($myrow = mysql_fetch_array($result)) { do {	
	printf ("<tr><td class=mastoxic><img src=tm/%s></td><td class=mascont align=center>%s</td><td class=mascont  align=center>%s - %s</td><td class=mascont  align=center><b>%s</td></tr>", urlencode($myrow["sm_flagfn"]), $myrow["sm_gp"],$myrow["sm_gf"],$myrow["sm_ga"],$myrow["sm_points"]);
...
<?php } else { ?><script>parent.location='m3_noid.php'</script><?php }  include ("sm_copy.php");?>
