...
<table width="100%" border="0" cellspacing="1" cellpadding="1">
 <tr>
  ...
  <?php
$result=MYSQL_QUERY("SELECT  tc_mid, tc_team1, tc_team2,  tc_g1, tc_g2, UNIX_TIMESTAMP(tc_date) as ladata, tc_group FROM te12_match, te12_teams WHERE tc_tid=tc_team1 AND tc_play='Y' ORDER BY tc_date");
if ($myrow = mysql_fetch_array($result)) {
        do{$gole="";
$xpu=""; $matchid=$myrow["tc_mid"];$sijuega= date("d/m/Y H:i", $myrow["ladata"]);
        $result2=MYSQL_QUERY("SELECT  tc_name1 FROM te12_goal, te12_stars WHERE tc_gmid=$matchid AND tc_sid=tc_gsid ORDER BY tc_name1");
        ...
        $result2=MYSQL_QUERY("SELECT  tc_name1 FROM te12_sentoff, te12_stars WHERE tc_smid=$matchid AND tc_sid=tc_ssid ORDER BY tc_name1");
        ...
        if ($myrow["ladata"]<mktime (0,0,0,6,20,2012)) {$whatmatch="Fase I - Gruppo " . $myrow["tc_group"];} else
        if ($myrow["ladata"]<mktime (0,0,0,6,26,2012)){$whatmatch="1/4 finale"  . $myrow["tc_group2"];} else
        if ($myrow["ladata"]<mktime (0,0,0,6,29,2012)){$whatmatch="semifinale";}else
 {$whatmatch="finale";}
...
$tim2 = $myrow["tc_team2"];
  ...
  <?php
$result=MYSQL_QUERY("SELECT  tc_team1, tc_team2, UNIX_TIMESTAMP(tc_date) as ladata, tc_group FROM te12_match, te12_teams WHERE tc_tid=tc_team1 AND tc_play='N' ORDER BY tc_date");
if ($myrow = mysql_fetch_array($result)) {
        do{$sijuega= date("d/m/Y H:i", $myrow["ladata"]);
        if ($myrow["ladata"]<mktime (0,0,0,6,20,2012)) {$whatmatch="Fase I - Gruppo " . $myrow["tc_group"];} else
        if ($myrow["ladata"]<mktime (0,0,0,6,26,2012)){$whatmatch="1/4 finale";}else
        if ($myrow["ladata"]<mktime (0,0,0,6,29,2012)){$whatmatch="semifinale";}else {$whatmatch="finale";}
        printf ("<tr><td class=mascont>%s</td><td class=mascont>%s</td>", $sijuega,$whatmatch);
                $tim1 = $myrow["tc_team1"];
$tim2 = $myrow["tc_team2"];
    $query1res = @MYSQL_QUERY("SELECT tc_flagfn FROM te12_teams WHERE tc_tid='$tim1'");
...
<?php
$result=MYSQL_QUERY("SELECT  tc_tid, tc_flagfn, tc_gf, tc_ga, tc_gp, tc_points, (tc_gf-tc_ga) as dr FROM te12_teams WHERE tc_group='A' ORDER BY tc_points DESC, dr DESC, tc_gf DESC,  tc_tid");
...
<?php
$result=MYSQL_QUERY("SELECT  tc_tid, tc_flagfn, tc_gf, tc_ga, tc_gp, tc_points, (tc_gf-tc_ga) as dr FROM te12_teams WHERE tc_group='B' ORDER BY tc_points DESC, dr DESC, tc_gf DESC,  tc_tid");
if ($myrow = mysql_fetch_array($result)) { do {
        printf ("<tr><td class=mastoxic><img src=ti/%s></td><td class=mascont align=center>%s</td><td class=mascont  align=center>%s - %s</td><td class=mascont  align=center><b>%s</td></tr>", urlencode($myrow["tc_flagfn"]), $myrow["tc_gp"],$myrow["tc_gf"],$myrow["tc_ga"],$myrow["tc_points"]);
...
<?php
$result=MYSQL_QUERY("SELECT  tc_tid, tc_flagfn, tc_gf, tc_ga, tc_gp, tc_points, (tc_gf-tc_ga) as dr FROM te12_teams WHERE tc_group='C' ORDER BY tc_points DESC, dr DESC, tc_gf DESC, tc_tid");
if ($myrow = mysql_fetch_array($result)) { do {
        printf ("<tr><td class=mastoxic><img src=ti/%s></td><td class=mascont align=center>%s</td><td class=mascont  align=center>%s - %s</td><td class=mascont  align=center><b>%s</td></tr>", urlencode($myrow["tc_flagfn"]), $myrow["tc_gp"],$myrow["tc_gf"],$myrow["tc_ga"],$myrow["tc_points"]);
...
<?php
$result=MYSQL_QUERY("SELECT  tc_tid, tc_flagfn, tc_gf, tc_ga, tc_gp, tc_points, (tc_gf-tc_ga) as dr FROM te12_teams WHERE tc_group='D' ORDER BY tc_points DESC, dr DESC, tc_gf DESC, tc_tid");
if ($myrow = mysql_fetch_array($result)) { do {
        printf ("<tr><td class=mastoxic><img src=ti/%s></td><td class=mascont align=center>%s</td><td class=mascont  align=center>%s - %s</td><td class=mascont  align=center><b>%s</td></tr>", urlencode($myrow["tc_flagfn"]), $myrow["tc_gp"],$myrow["tc_gf"],$myrow["tc_ga"],$myrow["tc_points"]);
...
<?php } else { ?><script>parent.location='te12_noid.php'</script><?php }  include ("te12_copy.php");?>
