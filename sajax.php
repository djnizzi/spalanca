   ...
   return text.replace(/\+/g, "%2B"); 
		...
		function sajax_do_call(func_name, args) {
			...
			if (sajax_request_type == "GET") {
				...
				if (uri.indexOf("?") == -1) 
					uri += "?rs=" + sajax_escape(func_name);
				else
					uri += "&rs=" + sajax_escape(func_name);
				uri += "&rst=" + sajax_escape(sajax_target_id);
				uri += "&rsrnd=" + new Date().getTime();
				
				for (i = 0; i < args.length-1; i++) 
					uri += "&rsargs[]=" + sajax_escape(args[i]);

				post_data = null;
			} 
			else if (sajax_request_type == "POST") {
				post_data = "rs=" + sajax_escape(func_name);
				post_data += "&rst=" + sajax_escape(sajax_target_id);
				post_data += "&rsrnd=" + new Date().getTime();
				
				for (i = 0; i < args.length-1; i++) 
					post_data = post_data + "&rsargs[]=" + sajax_escape(args[i]);
			}
			else {
