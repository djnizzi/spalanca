...
$people=$people."<a href=javascript:scheda('scheda.php?zid=".urlencode($myrow["id"])."');>".$myrow["id"]."</a><br>";
 ...
 $cp->set_data($people);
  ...
  $cp->register('add_line');
  $cp->register('get_lines');
  $cp->register('get_people');
  $cp->register('logitout');
  $cp->start();
  $cp->return_data(); 
	?>
<head>
...
<script src=mashiro.js></script>
  ...
  cp.set_debug(2);
	...
	function refresh_cb(cpr) {
		   
		document.getElementById("wall").innerHTML = cpr;
		setTimeout("refresh()", 1000);
	}
	
	function refresh() {
			cp.call('sm_chat07.php', 'get_lines', refresh_cb, '<? echo $fuso; ?>');
		
	}
		function p_refresh_cb(cpr) {
		document.getElementById("peop").innerHTML = cpr;
		setTimeout("p_refresh()", 1000);
	}
	
	function p_refresh() {
		cp.call('sm_chat07.php', 'get_people', p_refresh_cb);
	...
	function logmeout() {
		cp.call('sm_chat07.php', 'logitout', logmeout_cb, '<? echo $id; ?>');
	...
	function add() {
		...
		if (line == "") 
			return;
					cp.call('sm_chat07.php', 'add_line', add_cb, '<? echo $id; ?>', document.getElementById('line').value);
