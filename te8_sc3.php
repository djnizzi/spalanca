...
if (@MYSQL_RESULT($result,0,"tc_points")==null){
 ?>  
      <tr>
    <td class="mastoxic" align=center>questo personaggio non ha ancora compilato la scheda di partecipazione al <b>TOTOIURO08</b>
     </td></tr></table>
 <?php } else {   ?><tr>
 ...
 <?php
$result=MYSQL_QUERY("SELECT tc_mid, tc_team1, tc_team2,  te8_res.tc_g1 as g1, te8_res.tc_g2 as g2 , te8_res.tc_1x2 as x2, UNIX_TIMESTAMP(tc_date)  as ladata, tc_group FROM te8_match, te8_teams, te8_res WHERE tc_tid=tc_team1 AND tc_rmid=tc_mid AND tc_rpid='$zid' ORDER BY tc_date");
...
$tim2 = $myrow["tc_team2"];
	...
	<?php
$result2=MYSQL_QUERY("SELECT tc_16tid, tc_flagfn FROM te8_16teams, te8_teams WHERE  tc_16tid=tc_tid AND tc_16pid='$zid' AND tc_16w='4' ORDER BY tc_16tid");
...
printf ("<tr><td  class=mastoxic2 align=right width=50%%><img src=ti/%s></td><td  class=mastoxic2 width=50%%>%s</td>",urlencode($myrow["tc_flagfn"]),$myrow["tc_16tid"]);
	...
	<?php
$result2=MYSQL_QUERY("SELECT tc_16tid, tc_flagfn FROM te8_16teams, te8_teams WHERE  tc_16tid=tc_tid AND tc_16pid='$zid' AND tc_16w='2' ORDER BY tc_16tid");
if ($myrow = mysql_fetch_array($result2)) {
	do{
printf ("<tr><td  class=mastoxic2 align=right width=50%%><img src=ti/%s></td><td  class=mastoxic2 width=50%%>%s</td>",urlencode($myrow["tc_flagfn"]),$myrow["tc_16tid"]);
} while  ($myrow = mysql_fetch_array($result2));} ?>
	...
	<?php
$result2=MYSQL_QUERY("SELECT tc_16tid, tc_flagfn FROM te8_16teams, te8_teams WHERE  tc_16tid=tc_tid AND tc_16pid='$zid' AND tc_16w='F' ORDER BY tc_16tid");
if ($myrow = mysql_fetch_array($result2)) {
	do{
printf ("<tr><td  class=mastoxic2 align=right width=50%%><img src=ti/%s></td><td  class=mastoxic2 width=50%%>%s</td>",urlencode($myrow["tc_flagfn"]),$myrow["tc_16tid"]);
} while  ($myrow = mysql_fetch_array($result2));} ?>
	...
	<?php
$result2=MYSQL_QUERY("SELECT tc_16tid, tc_flagfn FROM te8_16teams, te8_teams WHERE  tc_16tid=tc_tid AND tc_16pid='$zid' AND tc_16w='W' ORDER BY tc_16tid");
if ($myrow = mysql_fetch_array($result2)) {
	do{
printf ("<tr><td  class=mastoxic2 align=right width=50%%><img src=ti/%s></td><td  class=mastoxic2 width=50%%>%s</td>",urlencode($myrow["tc_flagfn"]),$myrow["tc_16tid"]);
} while  ($myrow = mysql_fetch_array($result2));} ?>
	...
	<?php
$result2=MYSQL_QUERY("SELECT tc_name1 FROM te8_goleadors, te8_stars WHERE tc_gpid='$zid' AND tc_gsid=tc_sid ORDER BY tc_name1");
...
printf ("%s<br>",$myrow["tc_name1"]);
	...
	<?php
$result2=MYSQL_QUERY("SELECT tc_name1 FROM te8_final, te8_stars WHERE tc_fpid='$zid' AND tc_fsid=tc_sid ORDER BY tc_name1");
if ($myrow = mysql_fetch_array($result2)) {
	do{
