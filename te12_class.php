...
<table width="100%" border="0" cellspacing="1" cellpadding="1">
 <tr>
  ...
  <?php
  $result=MYSQL_QUERY("SELECT count(*) as segnari FROM te12_playaz");
  $partecipantes= @MYSQL_RESULT($result,0,"segnari");
  $result=MYSQL_QUERY("SELECT tc_points, tc_pid FROM te12_playaz ORDER BY tc_points DESC");
if ($myrow = mysql_fetch_array($result)) {
	do{
printf("<tr><td class=masblack valign=top><a href=te12_scheda.php?zid=%s>%s</a></td><td background=images/te12_bk.jpg align=right ><a href=te12_scheda.php?zid=%s></td><td class=mascont align=center><b>%s</b></td></tr>",urlencode($myrow["tc_pid"]),$myrow["tc_pid"],urlencode($myrow["tc_pid"]),$myrow["tc_points"]);
...
<?php	
$result=MYSQL_QUERY("SELECT count(*) as segnari, tc_16tid, tc_flagfn FROM te12_16teams, te12_teams WHERE  tc_16tid=tc_tid AND tc_16w='W'  GROUP BY tc_16tid  ORDER BY segnari DESC");
if ($myrow = mysql_fetch_array($result)) {
	do{
...
printf("<tr><td class=mastoxic align=right width=100><img src=ti/%s></td><td class=mascont width=104><img src=images/white.gif  width=%s height=14></td><td class=mascont width=50 align=right>%s %%</td></tr>",urlencode($myrow["tc_flagfn"]),round($perc,0),round($perc,2));
 ...
 <?php	
$result=MYSQL_QUERY("SELECT count(*) as segnari, tc_name1 FROM te12_goleadors, te12_stars WHERE tc_sid=tc_gsid  GROUP BY tc_gsid  ORDER BY segnari DESC LIMIT 10");
if ($myrow = mysql_fetch_array($result)) {
	do{
