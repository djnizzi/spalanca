...
<table width="100%" border="0" cellspacing="1" cellpadding="1">
 <tr>
  ...
  <?php
  $result=MYSQL_QUERY("SELECT count(*) as segnari FROM m3_playaz");
  $partecipantes= @MYSQL_RESULT($result,0,"segnari");
  $result=MYSQL_QUERY("SELECT sm_points, sm_pid FROM m3_playaz ORDER BY sm_points DESC");
if ($myrow = mysql_fetch_array($result)) {
	do{
printf("<tr><td class=masblack valign=top><a href=m3_scheda.php?zid=%s>%s</a></td><td background=images/m2_bk.jpg align=right width=78><a href=m3_scheda.php?zid=%s><img src=getdata.php?zid=%s&wht=users border=0  width=50 height=50></a></td><td class=mascont align=center><b><font size=+3>%s</font></b><br>punti</td></tr>",urlencode($myrow["sm_pid"]),$myrow["sm_pid"],urlencode($myrow["sm_pid"]),urlencode($myrow["sm_pid"]),$myrow["sm_points"]);
...
<?php	
$result=MYSQL_QUERY("SELECT count(*) as segnari, sm_16tid, sm_flagfn FROM m3_16teams, m3_teams WHERE sm_16tid=sm_tid AND sm_16w='W'  GROUP BY sm_16tid  ORDER BY segnari DESC");
if ($myrow = mysql_fetch_array($result)) {
	do{
...
printf("<tr><td class=mastoxic align=right width=100><img src=tm/%s></td><td class=mascont width=104><img src=images/white.gif  width=%s height=14></td><td class=mascont width=50 align=right>%s %%</td></tr>",urlencode($myrow["sm_flagfn"]),round($perc,0),round($perc,2));
 ...
 <?php	
$result=MYSQL_QUERY("SELECT count(*) as segnari, sm_name1 FROM m3_goleadors, m3_stars WHERE sm_sid=sm_gsid  GROUP BY sm_gsid  ORDER BY segnari DESC LIMIT 10");
if ($myrow = mysql_fetch_array($result)) {
	do{
...
<?php } else { ?><script>parent.location='m3_noid.php'</script><?php }   include ("m3_copy.php"); ?>
