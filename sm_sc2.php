...
<form name=scheda method=post action=sm_scheda.php?zid=<?php echo urlencode($id); ?>  onSubmit="return verScheda(document.scheda);">
...
$result=MYSQL_QUERY("SELECT sm_mid, sm_team1, sm_team2, UNIX_TIMESTAMP(DATE_ADD(sm_date, INTERVAL -7 HOUR)) as ladata, sm_group FROM sm_match, sm_teams WHERE sm_tid=sm_team1 ORDER BY sm_date");
if ($myrow = mysql_fetch_array($result)) {
	do{ $sijuega= date("d/m/Y H:i", $myrow["ladata"]);
$resultd=MYSQL_QUERY("SELECT  sm_g1, sm_g2, sm_1x2 FROM sm_res WHERE sm_rpid='$id' AND sm_rmid='{$myrow['sm_mid']}'"); 
...
$allteams=$allteams . "<option value='" . $myrow["sm_tid"] . "'>". $myrow["sm_tid"] . "</option>";
	...
	$result4=MYSQL_QUERY("SELECT sm_16tid FROM sm_16teams WHERE sm_16pid='$id'  AND sm_16w='8' ORDER BY sm_16tid");
if ($myrow2 = mysql_fetch_array($result4)) {
	do{
$my16=$my16 . "<option value='" . $myrow2["sm_16tid"] . "'>". $myrow2["sm_16tid"] . "</option>";
...
?>
	...
	<?php
		$result4=MYSQL_QUERY("SELECT sm_16tid FROM sm_16teams WHERE sm_16pid='$id'  AND sm_16w='4' ORDER BY sm_16tid");
if ($myrow2 = mysql_fetch_array($result4)) {
	do{
...
?>
	...
	<?php
		$result4=MYSQL_QUERY("SELECT sm_16tid FROM sm_16teams WHERE sm_16pid='$id'  AND sm_16w='2' ORDER BY sm_16tid");
if ($myrow2 = mysql_fetch_array($result4)) {
	do{
...
?>
	...
	<?php
		$result4=MYSQL_QUERY("SELECT sm_16tid FROM sm_16teams WHERE sm_16pid='$id'  AND sm_16w='F' ORDER BY sm_16tid");
if ($myrow2 = mysql_fetch_array($result4)) {
	do{
...
?>
	...
	<?php
		$result4=MYSQL_QUERY("SELECT sm_16tid FROM sm_16teams WHERE sm_16pid='$id'  AND sm_16w='W' ORDER BY sm_16tid");
if ($myrow2 = mysql_fetch_array($result4)) {
	do{
...
$result=MYSQL_QUERY("SELECT  sm_tid FROM sm_teams ORDER BY sm_tid");
...
do{
	...
	$resulto=MYSQL_QUERY("SELECT  sm_sid, sm_name1 FROM sm_stars WHERE sm_stid='{$myrow['sm_tid']}' ORDER BY sm_name1");
...
print("}</script>");} 
$result3=MYSQL_QUERY("SELECT sm_sid, sm_name1 FROM sm_stars ORDER BY sm_name1");
...
?>
	...
	<?php
	$result4=MYSQL_QUERY("SELECT sm_gsid,sm_name1 FROM sm_goleadors,sm_stars WHERE sm_gpid='$id' AND sm_gsid=sm_sid ORDER BY sm_name1");
if ($myrow2 = mysql_fetch_array($result4)) {
	do{
...
?>
	...
	<?php
$result4=MYSQL_QUERY("SELECT sm_fsid, sm_name1 FROM sm_final,sm_stars WHERE sm_fpid='$id' AND sm_fsid=sm_sid");
if ($myrow2 = mysql_fetch_array($result4)) {
	do{
