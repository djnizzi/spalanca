...
<table width="100%" border="0" cellspacing="1" cellpadding="1">
 <tr>
    ...
    <td class="mastoxic" align=center><table border="0" cellpadding="3" cellspacing=1 align=center class=masblack>
     <tr>
  <?php
  $result=MYSQL_QUERY("SELECT sm_points FROM m3_playaz WHERE sm_pid='$zid'");
...
print("	    <td class=masblack valign=top>$zid</td><td background=images/m3_bk.jpg align=right width=78>". $avatarurl ."</td><td class=mascont align=center><b><font size=+3>$sm_points</font></b><br>punti</td>");
    ?></tr></table></td></tr>
 <?php 
   $result=MYSQL_QUERY("SELECT sm_points FROM m3_playaz WHERE sm_pid='$zid'");
if (@MYSQL_RESULT($result,0,"sm_points")==null){
 ...
 <?php
$result=MYSQL_QUERY("SELECT sm_mid, sm_team1, sm_team2,  m3_res.sm_g1 as g1, m3_res.sm_g2 as g2 , m3_res.sm_1x2 as x2, UNIX_TIMESTAMP(sm_date)  as ladata, sm_group FROM m3_match, m3_teams, m3_res WHERE sm_tid=sm_team1 AND sm_rmid=sm_mid AND sm_rpid='$zid' ORDER BY sm_date");
...
$tim2 = $myrow["sm_team2"];
	...
	<?php
$result2=MYSQL_QUERY("SELECT sm_16tid, sm_flagfn FROM m3_16teams, m3_teams WHERE  sm_16tid=sm_tid AND sm_16pid='$zid' AND sm_16w='8' ORDER BY sm_16tid");
...
printf ("<tr><td  class=mastoxic2 align=right width=50%%><img src=tm/%s></td><td  class=mastoxic2 width=50%%>%s</td>",urlencode($myrow["sm_flagfn"]),$myrow["sm_16tid"]);
	...
	<?php
$result2=MYSQL_QUERY("SELECT sm_16tid, sm_flagfn FROM m3_16teams, m3_teams WHERE  sm_16tid=sm_tid AND sm_16pid='$zid' AND sm_16w='4' ORDER BY sm_16tid");
if ($myrow = mysql_fetch_array($result2)) {
	do{
printf ("<tr><td  class=mastoxic2 align=right width=50%%><img src=tm/%s></td><td  class=mastoxic2 width=50%%>%s</td>",urlencode($myrow["sm_flagfn"]),$myrow["sm_16tid"]);
} while  ($myrow = mysql_fetch_array($result2));} ?>
	...
	<?php
$result2=MYSQL_QUERY("SELECT sm_16tid, sm_flagfn FROM m3_16teams, m3_teams WHERE  sm_16tid=sm_tid AND sm_16pid='$zid' AND sm_16w='2' ORDER BY sm_16tid");
if ($myrow = mysql_fetch_array($result2)) {
	do{
printf ("<tr><td  class=mastoxic2 align=right width=50%%><img src=tm/%s></td><td  class=mastoxic2 width=50%%>%s</td>",urlencode($myrow["sm_flagfn"]),$myrow["sm_16tid"]);
} while  ($myrow = mysql_fetch_array($result2));} ?>
	...
	<?php
$result2=MYSQL_QUERY("SELECT sm_16tid, sm_flagfn FROM m3_16teams, m3_teams WHERE  sm_16tid=sm_tid AND sm_16pid='$zid' AND sm_16w='F' ORDER BY sm_16tid");
if ($myrow = mysql_fetch_array($result2)) {
	do{
printf ("<tr><td  class=mastoxic2 align=right width=50%%><img src=tm/%s></td><td  class=mastoxic2 width=50%%>%s</td>",urlencode($myrow["sm_flagfn"]),$myrow["sm_16tid"]);
} while  ($myrow = mysql_fetch_array($result2));} ?>
	...
	<?php
$result2=MYSQL_QUERY("SELECT sm_16tid, sm_flagfn FROM m3_16teams, m3_teams WHERE  sm_16tid=sm_tid AND sm_16pid='$zid' AND sm_16w='W' ORDER BY sm_16tid");
if ($myrow = mysql_fetch_array($result2)) {
	do{
printf ("<tr><td  class=mastoxic2 align=right width=50%%><img src=tm/%s></td><td  class=mastoxic2 width=50%%>%s</td>",urlencode($myrow["sm_flagfn"]),$myrow["sm_16tid"]);
} while  ($myrow = mysql_fetch_array($result2));} ?>
	...
	<?php
$result2=MYSQL_QUERY("SELECT sm_name1 FROM m3_goleadors, m3_stars WHERE sm_gpid='$zid' AND sm_gsid=sm_sid ORDER BY sm_name1");
...
printf ("%s<br>",$myrow["sm_name1"]);
	...
	<?php
$result2=MYSQL_QUERY("SELECT sm_name1 FROM m3_final, m3_stars WHERE sm_fpid='$zid' AND sm_fsid=sm_sid ORDER BY sm_name1");
if ($myrow = mysql_fetch_array($result2)) {
	do{
