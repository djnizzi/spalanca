...
<?php   include ("m2_nav.php");?>
<form name=scheda method=post action=m2_scheda.php?zid=<?php echo urlencode($id); ?>  onSubmit="return verScheda(document.scheda);">
...
$result=MYSQL_QUERY("SELECT sm_mid, sm_team1, sm_team2, UNIX_TIMESTAMP(sm_date) as ladata, sm_group FROM m2_match, m2_teams WHERE sm_tid=sm_team1 ORDER BY sm_date");
if ($myrow = mysql_fetch_array($result)) {
	do{ $sijuega= date("d/m/Y H:i", $myrow["ladata"]);
$resultd=MYSQL_QUERY("SELECT  sm_g1, sm_g2, sm_1x2 FROM m2_res WHERE sm_rpid='$id' AND sm_rmid='{$myrow['sm_mid']}'"); 
...
$optionx='<option selected value=' . $myrow2["sm_1x2"] .'>' .$myrow2["sm_1x2"] . '</option>';
	...
	printf ("<td nowrap class=mascont align=center><select  class=mastoxic name=sm_g1[%s]>$goalsstuff$optiong1</select> - <select class=mastoxic name=sm_g2[%s]>$goalsstuff$optiong2</select></td><td class=mascont align=center><select class=mastoxic name=sm_1x2[%s]>$signstuff$optionx</select></td></tr>", $myrow["sm_mid"],$myrow["sm_mid"],$myrow["sm_mid"] );
    ...
    <td class="mastoxic" align=center><table width="90%" border="0" cellspacing="1" cellpadding="1" align=center>
     <tr>
    <td class="masblack" align="center" colspan=3><img src="images/sm_sc2.jpg"></td>
  </tr>
	<?php
$result2=MYSQL_QUERY("SELECT sm_tid, sm_flagfn FROM m2_teams ORDER BY sm_tid");
if ($myrow = mysql_fetch_array($result2)) {
	do{
$allteams=$allteams . "<option value='" . htmlentities($myrow["sm_tid"], ENT_QUOTES) . "'>". $myrow["sm_tid"] . "</option>";
$allfnteams=$allfnteams . "<option value='" . $myrow["sm_flagfn"] . "'>". $myrow["sm_tid"] . "</option>";

} while  ($myrow = mysql_fetch_array($result2));

	$result4=MYSQL_QUERY("SELECT sm_16tid FROM m2_16teams WHERE sm_16pid='$id'  AND sm_16w='8' ORDER BY sm_16tid");
if ($myrow2 = mysql_fetch_array($result4)) {
	do{
$my16=$my16 . "<option value='" . htmlentities($myrow2["sm_16tid"], ENT_QUOTES) . "'>". $myrow2["sm_16tid"] . "</option>";
...
?>
	...
	<?php
		$result4=MYSQL_QUERY("SELECT sm_16tid FROM m2_16teams WHERE sm_16pid='$id'  AND sm_16w='4' ORDER BY sm_16tid");
if ($myrow2 = mysql_fetch_array($result4)) {
	do{
$my8=$my8 . "<option value='" . htmlentities($myrow2["sm_16tid"], ENT_QUOTES) . "'>". $myrow2["sm_16tid"] . "</option>";
} while  ($myrow2 = mysql_fetch_array($result4));}
...
?>
	...
	<?php
		$result4=MYSQL_QUERY("SELECT sm_16tid FROM m2_16teams WHERE sm_16pid='$id'  AND sm_16w='2' ORDER BY sm_16tid");
if ($myrow2 = mysql_fetch_array($result4)) {
	do{
$my4=$my4 . "<option value='" . htmlentities($myrow2["sm_16tid"], ENT_QUOTES) . "'>". $myrow2["sm_16tid"] . "</option>";
} while  ($myrow2 = mysql_fetch_array($result4));}
...
?>
	...
	<?php
		$result4=MYSQL_QUERY("SELECT sm_16tid FROM m2_16teams WHERE sm_16pid='$id'  AND sm_16w='F' ORDER BY sm_16tid");
if ($myrow2 = mysql_fetch_array($result4)) {
	do{
$my2=$my2 . "<option value='" . htmlentities($myrow2["sm_16tid"], ENT_QUOTES) . "'>". $myrow2["sm_16tid"] . "</option>";
} while  ($myrow2 = mysql_fetch_array($result4));}
...
?>
	...
	<?php
		$result4=MYSQL_QUERY("SELECT sm_16tid FROM m2_16teams WHERE sm_16pid='$id'  AND sm_16w='W' ORDER BY sm_16tid");
if ($myrow2 = mysql_fetch_array($result4)) {
	do{
$my1=$my1 . "<option value='" . htmlentities($myrow2["sm_16tid"], ENT_QUOTES) . "'>". $myrow2["sm_16tid"] . "</option>";
} while  ($myrow2 = mysql_fetch_array($result4));}
...
$result=MYSQL_QUERY("SELECT  sm_tid, sm_flagfn FROM m2_teams ORDER BY sm_tid");
...
do{
	...
	$resulto=MYSQL_QUERY("SELECT  sm_sid, sm_name1 FROM m2_stars WHERE sm_stid=\"{$myrow['sm_tid']}\" ORDER BY sm_name1");
	if ($myrow2 = mysql_fetch_array($resulto)) {
		printf ("if (squadra.options[squadra.options.selectedIndex].value=='%s'){", $myrow["sm_flagfn"]);	
	do{
...
print("}</script>");} 
$result3=MYSQL_QUERY("SELECT sm_sid, sm_name1 FROM m2_stars ORDER BY sm_name1");
...
?>
	...
	<?php
	$result4=MYSQL_QUERY("SELECT sm_gsid,sm_name1 FROM m2_goleadors,m2_stars WHERE sm_gpid='$id' AND sm_gsid=sm_sid ORDER BY sm_name1");
if ($myrow2 = mysql_fetch_array($result4)) {
	do{
$myG=$myG . "<option value='" . $myrow2["sm_gsid"] . "'>". $myrow2["sm_name1"] . "</option>";
} while  ($myrow2 = mysql_fetch_array($result4));}
print ("<tr><td  width=33% class=mascont align=center><select name=selectSG class=mastoxic onChange='dynsel(document.all.selectSG,document.all.selectG);'><option>seleziona squadra</option>$allfnteams</select><br><select class=mastoxic name=selectG size=10 multiple>");
print ("</select></td><td  width=33% class=mastoxic align=center><input type=button class=buttons value='aggiungi --&gt;' ");
...
?>
	...
	<?php
$result4=MYSQL_QUERY("SELECT sm_fsid, sm_name1 FROM m2_final,m2_stars WHERE sm_fpid='$id' AND sm_fsid=sm_sid");
if ($myrow2 = mysql_fetch_array($result4)) {
	do{
$myX=$myX . "<option value='" . $myrow2["sm_fsid"] . "'>". $myrow2["sm_name1"] . "</option>";
} while  ($myrow2 = mysql_fetch_array($result4));}
print ("<tr><td  width=33% class=mascont align=center><select name=selectXG class=mastoxic onChange='dynsel(document.all.selectXG,document.all.selectX);'><option>seleziona squadra</option>$allfnteams</select><br><select class=mastoxic name=selectX size=10 multiple>");
print ("</select></td><td width=33% class=mastoxic align=center><input type=button class=buttons value='aggiungi --&gt;' ");
