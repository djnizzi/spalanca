...
$result=MYSQL_QUERY("INSERT INTO concomments (content,cuid,cpid,vote,conid) ".
	...
	}else if (@MYSQL_RESULT($result0,0,"coniglio")>0){
		$result=MYSQL_QUERY("INSERT INTO concomments (content,cuid,cpid,vote,conid) ".
        "VALUES ('$content','$id','$entid','0','$conid')");}else{
...
if (isset($subro)) {
	...
	if($pbindata_name!="" && $pbindata_size<196605 && $pbindata_type=="image/pjpeg" || $pbindata_type=="image/jpeg"){
...
if (isset($entid)  && !isset($delro)){
	$result=MYSQL_QUERY("SELECT euid from contentries WHERE entid='$entid' AND conid='$conid'");
if ((isset($id) && $id==@MYSQL_RESULT($result,0,"euid")) || isset($subco)) {
 ?>
 <!- ->
 <?php  }
 else {
$result = mysql_query("UPDATE contentries SET eposted=eposted,entv=entv+1 WHERE entid='$entid'");	}
                        ...
                        <td bgcolor="#000000"><table width="100%" border="0" cellspacing="1" cellpadding="0">
                          <tr>
                            <td align="center" valign="middle" class="entry1"><table width="100%" border="0" cellpadding="0" cellspacing="0">
                                <tr>
                                  <td class="entry7"><img src="<?php echo $condir; ?>/spacer.gif"></td>
                                </tr>
                                <tr>
...
do {
	$vuterija=($myrow["vote"]==0)?"":$myrow["vote"];
printf("<tr><td class=votcomh nowrap><b>%s</b><br>%s</td><td class=votcomv bgcolor=%s>%s</td></tr><tr><td colspan=2 class=votcomc>%s</td></tr><tr><td colspan=2 class=whitepx><img src=%s/spacer.gif height=1 width=1></td></tr>",$myrow["cuid"],date("d/m/Y H:i",$myrow["cpostato"]),$voto[$myrow["vote"]],$vuterija,wordwrap($myrow["content"],20," ",1),$condir);
...
?>  
                          ...
                          <tr>
                            ...
                            <table border="0" cellspacing="0" cellpadding="0" width=100%><form method=post name=commenti action=conenter.php?conid=<?php echo $conid; ?>&entid=<?php echo $entid; ?>  onSubmit="return verComm(document.commenti);disBut(document.commenti.subco)">
                              <tr>
                                <td colspan="3"><textarea name="content" class="entfld2"></textarea></td>
                                </tr>
                              <tr>
                                <?php if(time()<$deadline) { ?><td colspan="3"><input name="vote" value=0 type="hidden" ><center><input name="subco" type="submit" class="entfld4" value="commenta"><?php }else{ ?><td><img src="<?php echo $condir; ?>/vs.gif" width="86" height="21" border=0 usemap="#vsmap"><td><input onFocus="this.blur()" name="vote" value=0 type="text" class="entfld1"></td>
                                <td><input name="subco" type="submit" class="entfld3" value="vota"><?php } ?></td>
                                </td>
                              </tr>
                            </form></table><?php } ?></td>
                          </tr>
